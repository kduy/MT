...
<join clause> ::= CROSS JOIN <stream reference>
				\alt [INNER] JOIN <stream  reference> (ON <predicate> | USING <field ident>)
...
\end{itemize}
Be aware that, if \textit{EVERY} clause is missing, the window is a tumbling window by default.
