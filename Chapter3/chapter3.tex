...
\end{itemize}
We extends the examples in~ \citep{Dindar:2013} with experiments on Flink in order to demonstrate that system with different execution model may produce different output , even with the same input and query. 
\begin{enumerate}
...
In count-based slide, supposed that $u_1$ is the system timestamp of last element of the first window, the last element $s<v,_,t_k>$ of the $k_{th}$ window must satisfies $|{s \in \mathbb{S}(t_{sys}): u_1 < s.t_{sys} \leq u_2}| = n.\omega_c$
